...
- Writes a generated stylesheet to `wp-content/uploads/fonts/etch-fonts.css` and falls back to inline CSS automatically when file delivery is unavailable.
...
- Enqueues the generated font stylesheet for the frontend, editor, and the plugin admin screen.
...
- Assign heading/body roles with `Save and apply everywhere` or `Save roles only`.
- Preview pairings in `Specimen`, `Card`, `Reading`, and `Interface` modes.
- Search the local library, inspect file details, save per-family fallback stacks, and delete unused families.
...
1. Download the latest release ZIP from [GitHub Releases](https://github.com/sathyvelukunashegaran/Etch-Custom-Fonts/releases).
2. In WordPress, go to `Plugins -> Add New Plugin -> Upload Plugin`.
3. Upload the ZIP, install it, and activate `Etch Custom Fonts`.
4. Open `Etch Fonts` in the WordPress admin menu.
...
3. Open `Etch Fonts` and click `Rescan fonts`.
...
3. In `Google search access`, open `Manage key`, paste your API key, and click `Save key`.
...
1. Open `Etch Fonts -> Add Font -> Google Fonts`.
...
3. Click `Save and apply everywhere` if you want the generated CSS to include role-based selectors for `body` and `h1` through `h6`.
4. Click `Save roles only` if you only want to keep the pairing and copy the generated snippets yourself.
5. Click `Open advanced tools` to access:
...
The plugin builds a generated stylesheet at `wp-content/uploads/fonts/etch-fonts.css`. When that file is current and writable, it is enqueued as a regular stylesheet and versioned with a `crc32b` hash of the generated CSS content.

If the file is missing, stale, or cannot be written, the plugin falls back to outputting the same CSS inline so the selected fonts still load. The generated CSS always includes the catalog's `@font-face` rules and can also include the heading/body usage rules when you save roles with `Save and apply everywhere`.

## No external dependencies

Etch Custom Fonts is a small hand-coded plugin with no Composer packages, npm packages, or bundled frontend libraries.
...
Etch Custom Fonts is translation-ready and uses the `etch-fonts` text domain.

A POT template is included at `languages/etch-fonts.pot`. You can use that file with tools like [Poedit](https://poedit.net/) or [Loco Translate](https://localise.biz/wordpress/plugin) to build `.po` and `.mo` translations for your site.
...
Etch Custom Fonts is licensed under the [GNU General Public License v2 or later](LICENSE).
